...
Board.prototype.getClickedTile = function(clickedX, clickedY) {
  return this.tiles.filter(tile => clickWithinTile(tile, clickedX, clickedY))[0];
};

Board.prototype.getTurnTiles = function() {
  return this.tiles.filter(tile => tile.constructor.name === 'PathTile' && tile.isTurnPoint);
};
...
Board.prototype.getTilesWithTowers = function() {
  return this.tiles.filter(tile => tile.type === 'buildTile' && !tile.vacant);
};
