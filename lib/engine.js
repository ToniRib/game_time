...
function drawTowers(ctx) {
  for (let tile of game.getTowerTiles()) {
    ctx.drawImage(images[tile.tower.type], tile.x, tile.y);
  }
}
...
function assignHeavyDamageTower() {
  let heavyDamageTower = new HeavyDamageTower({ x: currentTile.centerX(), y: currentTile.centerY() });

  if (tileIsAHighlightedBuildTile(currentTile) && canAffordTower(heavyDamageTower.price)) {
    currentTile.addTower(heavyDamageTower);
    currentTile.type = 'buildTile';
    $towerPrompt.hide();
    $clickBuildSitePrompt.show();
    game.monies -= heavyDamageTower.price;
  }
}

function assignContinousFireTower() {
  let continuousFireTower = new ContinuousFireTower({ x: currentTile.centerX(), y: currentTile.centerY() });

  if (tileIsAHighlightedBuildTile(currentTile) && canAffordTower(continuousFireTower.price)) {
    currentTile.addTower(continuousFireTower);
    currentTile.type = 'buildTile';
    $towerPrompt.hide();
    $clickBuildSitePrompt.show();
    game.monies -= continuousFireTower.price;
  }
}
...
heavyDamageTowerButton.addEventListener("click", assignHeavyDamageTower);
continuousFireTowerButton.addEventListener("click", assignContinousFireTower);
