...
function drawAnimations(ctx) {
  for (let animation of AnimationEngine.animations) {
    if(animation.nature === 'fire'){
      if (animation.tower.type === 'simpleTower' ||
          animation.tower.type === 'continuousFireTower' ||
          animation.tower.type === 'heavyDamageTower'){
        rotateAndDrawImage(ctx, animation);
      } else if (animation.tower.type === 'flashTower'){
        ctx.drawImage(animation.currentFrame,
                      animation.tower.x - (animation.tower.range / 2),
                      animation.tower.y - (animation.tower.range / 2));
      }
    } else if(animation.nature === 'hit' && animation.enemy !== null) {
      ctx.drawImage(animation.currentFrame, animation.enemy.x, animation.enemy.y - 11);
    }
  }
}
