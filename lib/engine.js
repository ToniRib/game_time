...
function drawAnimations(ctx) {
  for (let animation of animationEngine.liveAnimations()) {
    // if(animation.constructor.name === 'splash') {
    //   ctx.drawImage(animation.currentFrame, animation.tower.centerX(), animation.tower.centerY());
    // } else if (animation.constructor.name === 'direct') {
    ...
    //   rotateAndDrawImage (ctx, animation.enemy, animation.tower);
    // }
    rotateAndDrawImage(ctx, animation.enemy, animation.tower);
  }
}

function rotateAndDrawImage (ctx, enemy, tower) {
  let angleInRad = Math.atan2(enemy.y - tower.y, enemy.x, enemy.x);
  console.log(angleInRad);
  // ctx.translate( ctx.width/2, ctx.height/2 );
  // ctx.rotate( angleInRad );
  // ctx.drawImage( image, tower.x, tower.y );
  // ctx.rotate( -angleInRad );
...
function fireTowers(){
  let lastShotEvents = game.getTowers().forEach(tower => return tower.shoot(game.retrieveAliveEnemiesOnBoard()));
  triggerAnimation(lastShotEvents);
}

function triggerAnimation(shootEvent){
