...
/***/ function(module, exports, __webpack_require__) {
	...
	var simpleTowerButton = document.getElementById('simple-tower');
	var flashTowerButton = document.getElementById('flash-tower');
	var heavyDamageTowerButton = document.getElementById('heavy-damage-tower');
	var continuousFireTowerButton = document.getElementById('continuous-fire-tower');
	...
	function fireTowers() {
	  var shotEvents = [];
	  game.getTowers().forEach(function (tower) {
	    return shotEvents.push(tower.shoot(game.retrieveAliveEnemiesOnBoard()));
	  });
	  triggerAnimations(shotEvents);
	}

	function triggerAnimations(shootEvents) {
	  shootEvents.filter(function (event) {
	    return event !== undefined;
	  }).forEach(function (event) {
	    return animationEngine.triggerAnimation(event);
	  });
	}
	...
	function assignOrSelectTower() {
	  var tower = selectedTower || createTowerType(this.id);
...
/***/ function(module, exports, __webpack_require__) {
	...
	Game.prototype.rewardMonies = function () {
	  var deadEnemies = loDash.difference(this.enemiesInit, this.enemies);
	...
	Game.prototype.retrieveAliveEnemiesOnBoard = function () {
	  var enemies = this.retrieveAliveEnemies();
	...
	Game.prototype.changeEnemyDirectionAndReCenter = function (enemy, tile) {
	  var newTurnDirection = tile.turnDirection;
	  var originalDirection = enemy.currentDirection;
	...
	Game.prototype.resetClickedTiles = function () {
	  ...
	  this.board.getHighlightedOrSelectedBuildTiles().forEach(function (tile) {
	    return tile.type = 'buildTile';
	  });
	};

	Game.prototype.refreshGameObjectValues = function () {
	  this.getTowers().forEach(function (tower) {
	    return tower.fireRate = tower.fireRateRef;
	  });
	  this.enemies.forEach(function (enemy) {
	    return enemy.speed = enemy.speedRef;
	  });
	  this.board.getTurnTiles().forEach(function (tile) {
	    return tile.rangeBuffer = tile.rangeBufferRef;
	  });
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Level = function Level(level, difficulty) {
	  this.stage = level;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var boardTest = __webpack_require__(7);
	var boardOne = __webpack_require__(8);
...
/***/ function(module, exports) {
	...
	module.exports = boardTestOptions;
...
/***/ function(module, exports) {
	...
	var boardOneOptions = {
	  ...
	  65: { type: 'path', isTurn: false },
	  66: { type: 'path', isTurn: false },
	...
	module.exports = boardOneOptions;
...
/***/ function(module, exports) {
	...
	var boardTwoOptions = {
	  ...
	  71: { type: 'path', isTurn: true, direction: 'right' },
	  72: { type: 'path', isTurn: false },
	  73: { type: 'path', isTurn: false },
	  74: { type: 'path', isTurn: false },
...
/***/ function(module, exports, __webpack_require__) {
	...
	Board.prototype.getTurnTiles = function () {
	  return this.tiles.filter(function (tile) {
	    return tile.isPathTile() && tile.isTurnPoint;
	  });
	};

	Board.prototype.getHighlightedOrSelectedBuildTiles = function () {
	  return this.tiles.filter(function (tile) {
	    return tile.isHighlightedBuildTile() || tile.isSelectedBuildTile();
	  });
	};

	Board.prototype.removeAllTowers = function () {
	  this.getTilesWithTowers().forEach(function (tile) {
	    return tile.removeTower();
	  });
	};

	Board.prototype.getTilesWithTowers = function () {
	  return this.tiles.filter(function (tile) {
	    return tile.isBuildTile() && tile.hasTower();
	  });
	};
	...
	Board.prototype.currentTileIsAVacantBuildTile = function () {
	  return this.currentTile && this.currentTile.isBuildTile() && this.currentTile.vacant;
	};

	Board.prototype.lastTileIsHighlightedOrSelected = function () {
	  return this.lastTile && (this.lastTile.isHighlightedBuildTile() || this.lastTile.isSelectedBuildTile());
	};
	...
	Board.prototype.currentTileIsAHighlightedBuildTile = function () {
	  return this.currentTile && this.currentTile.isHighlightedBuildTile();
	};
	...
	Board.prototype.updateTileTypes = function () {
	  if (this.currentTileIsAVacantBuildTile() && this.lastTileIsHighlightedOrSelected()) {
	    ...
	    if (this.lastTileIsHighlightedOrSelected()) {
	      this.lastTile.type = 'buildTile';
	    }
	  }
	};

	var clickWithinTile = function clickWithinTile(tile, clickedX, clickedY) {
	  return withinXRange(tile.x, tile.x + tile.width, clickedX) && withinYRange(tile.y, tile.y + tile.height, clickedY);
	};

	var withinXRange = function withinXRange(left, right, value) {
	  return left <= value && value < right;
	};

	var withinYRange = function withinYRange(top, bottom, value) {
	  return top <= value && value < bottom;
	};

	var buildBoard = function buildBoard(tiles, options) {
	  var count = 0;
	  for (var y = 0; y < 10; y++) {
	    for (var x = 0; x < 16; x++) {
	...
	var tileDelegate = function tileDelegate(x, y, tiles, options, count) {
	  if (isPath(options, count)) {
	    tiles.push(new PathTile({ x: x * 50, y: y * 50 }, options[count].isTurn, options[count].direction));
	...
	var isPath = function isPath(options, count) {
	  return options[count] !== undefined && options[count].type === 'path';
	};

	var isBuild = function isBuild(options, count) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var enemiesOne = {
	  createEnemies: function createEnemies() {
	    return [new SimpleEnemy({ x: -100, y: 200 }), new SimpleEnemy({ x: -200, y: 200 }), new SimpleEnemy({ x: -300, y: 200 }), new SimpleEnemy({ x: -400, y: 200 }), new SimpleEnemy({ x: -550, y: 200 }), new SimpleEnemy({ x: -650, y: 200 })];
	  }
	};
...
/***/ function(module, exports) {
	...
	var SimpleEnemy = function SimpleEnemy(coord) {
	  this.x = coord.x;
	  this.y = coord.y;
	  this.speedRef = 1;
	  this.speed = 1;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var enemiesTwo = {
	  createEnemies: function createEnemies() {
	    return [new SimpleEnemy({ x: -100, y: 300 }), new SimpleEnemy({ x: -200, y: 300 }), new QuickEnemy({ x: -600, y: 300 }), new QuickEnemy({ x: -700, y: 300 }), new ToastEnemy({ x: -800, y: 300 }), new QuickEnemy({ x: -350, y: 300 }), new QuickEnemy({ x: -800, y: 300 }), new QuickEnemy({ x: -900, y: 300 }), new QuickEnemy({ x: -1000, y: 300 })];
	  }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var enemiesThree = {
	  createEnemies: function createEnemies() {
	    return [new SimpleEnemy({ x: -100, y: 50 }), new SimpleEnemy({ x: -200, y: 50 }), new MonsterEnemy({ x: -75, y: 50 }), new SimpleEnemy({ x: -300, y: 50 }), new SimpleEnemy({ x: -325, y: 50 }), new SimpleEnemy({ x: -375, y: 50 }), new SimpleEnemy({ x: -400, y: 50 }), new SimpleEnemy({ x: -435, y: 50 }), new SimpleEnemy({ x: -450, y: 50 }), new ToastEnemy({ x: -300, y: 50 }), new ToastEnemy({ x: -400, y: 50 }), new QuickEnemy({ x: -300, y: 50 }), new QuickEnemy({ x: -350, y: 50 }), new QuickEnemy({ x: -800, y: 50 }), new QuickEnemy({ x: -900, y: 50 }), new QuickEnemy({ x: -1000, y: 50 }), new QuickEnemy({ x: -1100, y: 50 }), new QuickEnemy({ x: -1200, y: 50 }), new QuickEnemy({ x: -1450, y: 50 }), new MonsterEnemy({ x: -500, y: 50 }), new MonsterEnemy({ x: -300, y: 50 }), new MonsterEnemy({ x: -600, y: 50 }), new ToastEnemy({ x: -900, y: 50 }), new ToastEnemy({ x: -1100, y: 50 }), new ToastEnemy({ x: -1300, y: 50 }), new SimpleEnemy({ x: -950, y: 50 }), new SimpleEnemy({ x: -1150, y: 50 }), new SimpleEnemy({ x: -1350, y: 50 }), new ToastEnemy({ x: -1600, y: 50 }), new ToastEnemy({ x: -1650, y: 50 }), new ToastEnemy({ x: -1800, y: 50 }), new ToastEnemy({ x: -1875, y: 50 }), new QuickEnemy({ x: -1450, y: 50 }), new QuickEnemy({ x: -1500, y: 50 }), new QuickEnemy({ x: -2000, y: 50 }), new QuickEnemy({ x: -2200, y: 50 }), new SimpleEnemy({ x: -1800, y: 50 }), new SimpleEnemy({ x: -1815, y: 50 }), new SimpleEnemy({ x: -1820, y: 50 }), new SimpleEnemy({ x: -1850, y: 50 }), new SimpleEnemy({ x: -1875, y: 50 }), new SimpleEnemy({ x: -1900, y: 50 }), new ToastEnemy({ x: -1900, y: 50 }), new ToastEnemy({ x: -1950, y: 50 }), new QuickEnemy({ x: -2300, y: 50 }), new QuickEnemy({ x: -2350, y: 50 }), new QuickEnemy({ x: -2400, y: 50 }), new QuickEnemy({ x: -2425, y: 50 }), new QuickEnemy({ x: -4500, y: 50 }), new QuickEnemy({ x: -4520, y: 50 }), new QuickEnemy({ x: -4530, y: 50 }), new QuickEnemy({ x: -4540, y: 50 }), new QuickEnemy({ x: -4550, y: 50 })];
	  }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var enemiesFour = {
	  createEnemies: function createEnemies() {
	    return [new ToastEnemy({ x: -700, y: 400 }), new ToastEnemy({ x: -725, y: 400 }), new ToastEnemy({ x: -750, y: 400 }), new ToastEnemy({ x: -1000, y: 400 }), new ToastEnemy({ x: -1500, y: 400 }), new ToastEnemy({ x: -2000, y: 400 }), new ToastEnemy({ x: -2500, y: 400 }), new ToastEnemy({ x: -3000, y: 400 }), new ToastEnemy({ x: -3500, y: 400 }), new SimpleEnemy({ x: -900, y: 400 }), new SimpleEnemy({ x: -926, y: 400 }), new SimpleEnemy({ x: -950, y: 400 }), new SimpleEnemy({ x: -975, y: 400 }), new SimpleEnemy({ x: -1000, y: 400 }), new SimpleEnemy({ x: -1050, y: 400 }), new SimpleEnemy({ x: -1000, y: 400 }), new SimpleEnemy({ x: -1020, y: 400 }), new SimpleEnemy({ x: -1100, y: 400 }), new SimpleEnemy({ x: -1150, y: 400 }), new SimpleEnemy({ x: -2400, y: 400 }), new SimpleEnemy({ x: -2450, y: 400 }), new SimpleEnemy({ x: -2500, y: 400 }), new SimpleEnemy({ x: -2550, y: 400 }), new SimpleEnemy({ x: -2600, y: 400 }), new SimpleEnemy({ x: -2650, y: 400 }), new MonsterEnemy({ x: -50, y: 400 }), new MonsterEnemy({ x: -150, y: 400 }), new MonsterEnemy({ x: -250, y: 400 }), new MonsterEnemy({ x: -350, y: 400 }), new MonsterEnemy({ x: -450, y: 400 }), new MonsterEnemy({ x: -550, y: 400 }), new MonsterEnemy({ x: -650, y: 400 }), new MonsterEnemy({ x: -750, y: 400 }), new MonsterEnemy({ x: -800, y: 400 }), new QuickEnemy({ x: -2400, y: 400 }), new QuickEnemy({ x: -2450, y: 400 }), new QuickEnemy({ x: -2500, y: 400 }), new QuickEnemy({ x: -2550, y: 400 }), new QuickEnemy({ x: -2525, y: 400 }), new QuickEnemy({ x: -2575, y: 400 }), new QuickEnemy({ x: -2600, y: 400 }), new QuickEnemy({ x: -2650, y: 400 }), new QuickEnemy({ x: -12675, y: 400 }), new QuickEnemy({ x: -12700, y: 400 }), new QuickEnemy({ x: -13000, y: 400 }), new QuickEnemy({ x: -13100, y: 400 }), new QuickEnemy({ x: -13300, y: 400 }), new QuickEnemy({ x: -13300, y: 400 }), new QuickEnemy({ x: -13400, y: 400 }), new QuickEnemy({ x: -13500, y: 400 }), new QuickEnemy({ x: -13600, y: 400 }), new QuickEnemy({ x: -14675, y: 400 }), new QuickEnemy({ x: -14700, y: 400 }), new QuickEnemy({ x: -14000, y: 400 }), new QuickEnemy({ x: -14100, y: 400 }), new QuickEnemy({ x: -14300, y: 400 }), new QuickEnemy({ x: -15300, y: 400 }), new QuickEnemy({ x: -15400, y: 400 }), new QuickEnemy({ x: -15500, y: 400 }), new QuickEnemy({ x: -15600, y: 400 }), new QuickEnemy({ x: -32000, y: 400 }), new QuickEnemy({ x: -32025, y: 400 }), new QuickEnemy({ x: -32050, y: 400 }), new QuickEnemy({ x: -32075, y: 400 }), new QuickEnemy({ x: -32100, y: 400 }), new QuickEnemy({ x: -32125, y: 400 }), new QuickEnemy({ x: -32150, y: 400 }), new QuickEnemy({ x: -32175, y: 400 }), new QuickEnemy({ x: -32200, y: 400 }), new QuickEnemy({ x: -32225, y: 400 }), new QuickEnemy({ x: -32250, y: 400 }), new QuickEnemy({ x: -32275, y: 400 }), new QuickEnemy({ x: -32300, y: 400 }), new QuickEnemy({ x: -32325, y: 400 }), new QuickEnemy({ x: -32350, y: 400 }), new QuickEnemy({ x: -32375, y: 400 }), new QuickEnemy({ x: -32425, y: 400 }), new QuickEnemy({ x: -32450, y: 400 }), new QuickEnemy({ x: -32475, y: 400 }), new QuickEnemy({ x: -32500, y: 400 }), new QuickEnemy({ x: -32525, y: 400 }), new QuickEnemy({ x: -32550, y: 400 }), new QuickEnemy({ x: -32575, y: 400 }), new QuickEnemy({ x: -32600, y: 400 }), new QuickEnemy({ x: -32625, y: 400 }), new QuickEnemy({ x: -32650, y: 400 }), new QuickEnemy({ x: -32675, y: 400 }), new QuickEnemy({ x: -32700, y: 400 }), new QuickEnemy({ x: -32725, y: 400 }), new QuickEnemy({ x: -32750, y: 400 }), new QuickEnemy({ x: -32775, y: 400 }), new QuickEnemy({ x: -32800, y: 400 }), new QuickEnemy({ x: -32825, y: 400 }), new QuickEnemy({ x: -32850, y: 400 }), new QuickEnemy({ x: -32875, y: 400 }), new QuickEnemy({ x: -32900, y: 400 }), new QuickEnemy({ x: -32925, y: 400 }), new QuickEnemy({ x: -32950, y: 400 }), new QuickEnemy({ x: -33000, y: 400 }), new QuickEnemy({ x: -33025, y: 400 }), new QuickEnemy({ x: -33050, y: 400 }), new QuickEnemy({ x: -33170, y: 400 })];
	  }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var enemiesTest = {
	  createEnemies: function createEnemies() {
	    return [new SimpleEnemy({ x: -100, y: 300 }), new SimpleEnemy({ x: -200, y: 300 }), new SimpleEnemy({ x: -300, y: 300 }), new QuickEnemy({ x: -600, y: 300 }), new QuickEnemy({ x: -700, y: 300 }), new QuickEnemy({ x: -500, y: 300 })];
	  }
	};
